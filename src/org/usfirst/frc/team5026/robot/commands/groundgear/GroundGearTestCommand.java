package org.usfirst.frc.team5026.robot.commands.groundgear;

import org.usfirst.frc.team5026.robot.Robot;
import org.usfirst.frc.team5026.util.Constants;
import org.usfirst.frc.team5026.util.GroundGearElevationState;

import com.ctre.CANTalon;
import com.ctre.CANTalon.FeedbackDevice;
import com.ctre.CANTalon.TalonControlMode;

import edu.wpi.first.wpilibj.command.Command;
import edu.wpi.first.wpilibj.smartdashboard.SmartDashboard;

/**
 *
 */
public class GroundGearTestCommand extends Command {

	CANTalon motor;
	GroundGearElevationState target;
	
    public GroundGearTestCommand(GroundGearElevationState target) {
        requires(Robot.groundgear);
        this.target = target;
        motor = Robot.groundgear.getElevationMotor();
        System.out.println(motor.getDeviceID());
    }

    // Called just before this Command runs the first time
    protected void initialize() {
//    	motor.reset();
    	motor.setFeedbackDevice(FeedbackDevice.QuadEncoder);
        motor.configEncoderCodesPerRev(1024); // if using FeedbackDevice.QuadEncoder
        motor.setProfile(0);
//        motor.setEncPosition(0);
        motor.setMotionMagicAcceleration(10);
        motor.setMotionMagicCruiseVelocity(5);
//    	Robot.hardware.groundGearLiftgroup.resetPosition();
//    	Robot.hardware.groundGearLiftgroup.setupProfileMode();
    }

    // Called repeatedly when this Command is scheduled to run
    protected void execute() {
    	motor.enable();
    	double motorOut = motor.getOutputVoltage() / motor.getBusVoltage();
    	
    	motor.changeControlMode(TalonControlMode.MotionMagic);
    	motor.set(target.rotations);
//    	Robot.hardware.groundGearLiftgroup.profileControl(10000);
    	
    	SmartDashboard.putNumber("GroundGearTarget", target.rotations);
    	SmartDashboard.putNumber("GroundGearSpeed", motor.getSpeed());
    	SmartDashboard.putNumber("GroundGearError", motor.getClosedLoopError());
        SmartDashboard.putNumber("GroundGearOutput", motorOut);
        SmartDashboard.putNumber("GroundGearPosition", motor.getPosition());
    }

    // Make this return true when this Command no longer needs to run execute()
    protected boolean isFinished() {
        return false;
    }

    // Called once after isFinished returns true
    protected void end() {
    	System.out.println("Ended!");
    }

    // Called when another command which requires one or more of the same
    // subsystems is scheduled to run
    protected void interrupted() {
    	System.out.println("Interrupted!");
    }
}
