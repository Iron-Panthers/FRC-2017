...
public class MotionProfilePath {
	public MotionProfileSegment[] points; // Keeps a list of all points
	public int currentIndex = 0; // Keeps track of the current profile point targetted (saves processing speed long term)

	public MotionProfilePath (MotionProfileSegment... p) {
		points = p;
	}
	public MotionProfileSegment getClosestPointOnPath(double targetX, double targetY) {
		double lastDistance = 10000; // Sets the last distance to a large number initially.
		MotionProfileSegment targetPoint = points[points.length-1]; // Uses MotionProfilePoint as a targetPoint, default last point
